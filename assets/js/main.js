(function() {
  ...
  // Lazy-load iframe on show; clear on hide to stop playback 
  document.addEventListener('show.bs.modal', (e) => {
    const modal = e.target;
    const iframe = modal.querySelector('iframe[data-src]');
    if (!iframe) return;
    iframe.src = iframe.dataset.src; // always set on open
  });
  ...
  document.addEventListener('hidden.bs.modal', (e) => {
    ...
    const iframe = modal.querySelector('iframe[data-src]');
    if (!iframe) return;
    iframe.removeAttribute('src'); // fully clear so it reloads next time
  });
